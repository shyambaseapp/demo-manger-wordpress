...
class DemoManager
{
 ...
 #Access database name from wp-config.php
                $regex = '/DB_NAME[\' ,"]+(.*?)[\'"]/';
                preg_match($regex, $wp_config, $return);
                $this->dbname = $return[1];
 #Access user name from wp-config.php
                $regex = '/DB_USER[\' ,"]+(.*?)[\'"]/';
                preg_match($regex, $wp_config, $return);
                $this->dbuser = $return[1];
 #Access user password from wp-config.php
                $regex = '/PASSWORD[\' ,"]+(.*?)[\'"]/';
                preg_match($regex, $wp_config, $return);
                $this->dbpass = $return[1];
 #Access host name from wp-config.php
                $regex = '/DB_HOST[\' ,"]+(.*?)[\'"]/';
                preg_match($regex, $wp_config, $return);
...
# 
    ...
    {
        $reset_destination = hex2bin(basename($backup_destination));
                ...
                try {
                    if (sizeof($argv) <= 2) {
                        ...
                        $demo_manager->read_wordpress_config($backup_destination);
                          try {
                                if ($demo_manager->check_last_login($reset_destination) < 24) {
